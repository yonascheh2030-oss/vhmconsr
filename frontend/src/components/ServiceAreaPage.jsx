...
export const ServiceAreaPage = () => {
  ...
  const service = SERVICE_INFO.find((s) => s.slug === serviceSlug);
  ...
  const pageUrl = `${SITE.domain}/${service.slug}/${area.slug}`;
  ...
  return (
    <main>
      <Helmet>
        <title>{`${service.name} ${area.name} — 24/7 | Sanivolt`}</title>
        <meta
          name="description"
          content={`${service.name} in ${area.name}: ${service.desc} Bij spoed ${area.time} ter plaatse. Bel ${SITE.phoneDisplay}.`}
        />
        <link rel="canonical" href={pageUrl} />
        <meta property="og:title" content={`${service.name} ${area.name} — 24/7 | Sanivolt`} />
        <meta property="og:description" content={`${service.desc} Bij spoed ${area.time} ter plaatse in ${area.name}.`} />
      ...
      <section className="relative pt-[70px] bg-brand-paper overflow-hidden">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10 pt-14 lg:pt-20 pb-20">
          ...
          >
            <Link to="/" className="hover:text-brand-blue transition-colors" data-testid="sa-breadcrumb-home">
              Home
            </Link>
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-brand-blue">{service.name}</span>
          </nav>
          ...
          <h1 className="font-outfit font-black uppercase tracking-tighter leading-[0.95] text-brand-ink text-[10vw] sm:text-6xl lg:text-7xl">
            <MaskLine delay={0.15}>{service.name}</MaskLine>
            <MaskLine delay={0.3}>
              in <span className="text-brand-blue">{area.name}.</span>
            </MaskLine>
          </h1>

          <MaskLine delay={0.5}>
            <p className="mt-8 max-w-2xl font-manrope text-base md:text-lg leading-relaxed text-zinc-500">
              {service.desc} {area.intro}
            </p>
          </MaskLine>
          ...
          <MaskLine delay={0.8}>
            <div className="mt-10 flex flex-wrap gap-x-8 gap-y-3">
              ...
              <span className="flex items-center gap-2 font-manrope text-xs font-semibold tracking-wide uppercase text-zinc-500">
                <BadgeCheck className="w-4 h-4 text-brand-blue" /> {service.price}
              </span>
            </div>
          </MaskLine>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          ...
          <Reveal>
            <h2 className="font-outfit font-black uppercase tracking-tighter text-2xl sm:text-3xl lg:text-4xl max-w-3xl">
              {service.name} in {area.name}: <span className="text-brand-volt">dit leveren wij</span>
            </h2>
          </Reveal>
          ...
          <Reveal delay={0.2}>
            <p className="mt-8 max-w-2xl font-manrope text-sm text-zinc-400 leading-relaxed">
              Transparante prijzen ({service.price}), vooraf een duidelijke offerte en achteraf 2
              jaar garantie. Voorrijkosten in {area.name}: vanaf €59, diagnose gratis bij
              uitgevoerde herstelling.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bg-white py-20 lg:py-28">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <ChapterTag number="02" label="Meer van Sanivolt" />
          <div className="grid lg:grid-cols-2 gap-12">
            <Reveal>
              <p className="font-manrope text-[11px] font-bold tracking-[0.25em] uppercase text-zinc-500 mb-6 flex items-center gap-2">
                <ArrowUpRight className="w-4 h-4 text-brand-blue" /> Andere diensten in {area.name}
              </p>
            ...
            <Reveal delay={0.1}>
              <p className="font-manrope text-[11px] font-bold tracking-[0.25em] uppercase text-zinc-500 mb-6 flex items-center gap-2">
                <MapPin className="w-4 h-4 text-brand-blue" /> {service.name} in andere gemeenten
              </p>
